# AI-Swarm — Контекст проекта

Используй этот файл как переиспользуемый контекст при работе с кодовой базой AI-Swarm. Вставь его в новый диалог и добавь свою цель.
...
- Backlog → перенаправление на Phase 0
- AI To Do → продолжение полного pipeline (или запуск чеклиста, если артефакты уже существуют)
- Другие статусы → запуск проверки статуса (валидация требований для текущего этапа)
...
- Ищет URL репозитория GitHub по источникам в порядке приоритета: описание Jira → комментарии назначенного → кастомное поле Jira (`project_link`) → Confluence Passport
...
При успешном завершении pipeline (`post_execution.py`):
...
Переходы выполняются автоматически системой на этапах Phase 0.5 → "AI To Do", Post-Execution → "Human Plan Review" и обработчик Plan Review → "Ready for Dev" (когда [PLAN REVIEW] в Done). Все переходы от Backlog до Ready for Dev управляются через `--task`. Остальные переходы (In Progress → Done) управляются командой вручную.

При запуске `--task` на задаче в статусе "Human Plan Review" система проверяет, находится ли задача [PLAN REVIEW] в статусе Done. Если утверждена — создаёт дочерние истории и переводит в "Ready for Dev". Если ещё не утверждена — информирует пользователя и завершается. Для статусов после "Ready for Dev" запускается `status_checker` — валидирует наличие необходимых артефактов для текущего статуса и автоисправляет недостающие.
...
7. Частичные сбои (часть историй создана, часть нет) дают exit code 2 с подробным отчётом — переход не выполняется
...
**Блокировка по задаче:** Файловая блокировка на каждый ключ задачи (`outputs/.locks/{KEY}.lock`). Предотвращает одновременный запуск pipeline на одну задачу. Используется `fcntl.LOCK_EX | LOCK_NB` — мгновенный отказ, если блокировку держит другой процесс.

**Атомарная запись файлов:** Все выходные файлы записываются через `atomic_write()` — запись в `.tmp`, затем `Path.replace()` (атомарная на POSIX). Предотвращает частичные записи при сбое или конкурентном доступе.

**Retry для LLM API:** Транзиентные ошибки (429, 502, 503, 504) вызывают экспоненциальный backoff: 2с → 4с → 8с, до 3 попыток. Нетранзиентные ошибки приводят к немедленному сбою. Настраивается через `sdlc_config.yaml`.

**Валидационный шлюз Phase 0:** Ответы LLM валидируются перед записью в Jira. Критические ошибки (отсутствие секций use_case или definition_of_ready целиком) блокируют запись в Jira — сырой ответ сохраняется в `outputs/` для отладки, но Jira не модифицируется. Некритические ошибки (отсутствие chain_of_thought) порождают предупреждения, но запись разрешена.
...
**Верификация переходов Jira:** После POST-запроса на переход статуса система проверяет, что задача действительно достигла целевого статуса, отлавливая гонки состояний от конкурентных модификаций.
...
```bash
# Полный pipeline (автомаршрутизация по статусу Jira)
...
| Модуль | Назначение |
|--------|-----------|
| `execute.py` | CLI точка входа, оркестрация pipeline, блокировка задач |
...
| `src/executor/phases/post_execution.py` | Пост-обработка: комментарии Jira, переходы статусов, создание задачи на ревью |
